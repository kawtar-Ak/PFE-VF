...
const detectCompetitionType = (league, matches) =>
  CUP_NAME_HINTS.some((hint) => normalize(league).includes(hint)) ||
  matches.some((match) => KNOCKOUT_HINTS.some((hint) => normalize(match?.round).includes(hint)))
    ? 'cup'
    : 'league';
const getScoreValue = (value) => (typeof value === 'number' ? value : Number.isFinite(Number(value)) ? Number(value) : null);
const getFormColor = (char, C) => (char === 'W' ? C.success : char === 'L' ? C.live : C.muted);
...
const buildFallbackTable = (matches) => {
  ...
  matches.forEach((match) => {
    ...
    if (!stats[home]) stats[home] = { rank: 0, team: { name: home, logo: match?.homeTeamLogo || null }, all: { played: 0, win: 0, draw: 0, lose: 0, goals: { for: 0, against: 0 } }, goalsDiff: 0, points: 0, form: '' };
    if (!stats[away]) stats[away] = { rank: 0, team: { name: away, logo: match?.awayTeamLogo || null }, all: { played: 0, win: 0, draw: 0, lose: 0, goals: { for: 0, against: 0 } }, goalsDiff: 0, points: 0, form: '' };
    const homeRow = stats[home];
    const awayRow = stats[away];
    homeRow.all.played += 1; awayRow.all.played += 1;
    homeRow.all.goals.for += homeScore; homeRow.all.goals.against += awayScore;
    awayRow.all.goals.for += awayScore; awayRow.all.goals.against += homeScore;
    if (homeScore > awayScore) { homeRow.all.win += 1; awayRow.all.lose += 1; homeRow.points += 3; homeRow.form = `W${homeRow.form}`.slice(0, 5); awayRow.form = `L${awayRow.form}`.slice(0, 5); }
    else if (awayScore > homeScore) { awayRow.all.win += 1; homeRow.all.lose += 1; awayRow.points += 3; awayRow.form = `W${awayRow.form}`.slice(0, 5); homeRow.form = `L${homeRow.form}`.slice(0, 5); }
    else { homeRow.all.draw += 1; awayRow.all.draw += 1; homeRow.points += 1; awayRow.points += 1; homeRow.form = `D${homeRow.form}`.slice(0, 5); awayRow.form = `D${awayRow.form}`.slice(0, 5); }
  ...
  return Object.values(stats)
    .sort((left, right) => right.points - left.points || right.goalsDiff - left.goalsDiff || right.all.goals.for - left.all.goals.for || (left.team?.name || '').localeCompare(right.team?.name || ''))
    .map((row, index) => ({ ...row, rank: index + 1 }));
};

const renderForm = (form, styles, C) => {
  const safe = String(form || '').toUpperCase().replace(/[^WDL]/g, '').slice(0, 5);
  if (!safe) return <Text style={styles.formEmpty}>-</Text>;
  return (
    <View style={styles.formWrap}>
      {safe.split('').map((char, index) => <View key={`${char}-${index}`} style={[styles.formDot, { backgroundColor: getFormColor(char, C) }]} />)}
    </View>
  );
};
...
export default function LeagueCompetitionScreen({ route, navigation }) {
  ...
  const sortedStandings = useMemo(() => standings.length > 0 ? [...standings].sort((left, right) => (left?.rank || 0) - (right?.rank || 0)) : buildFallbackTable(matches), [matches, standings]);
  const standingsByGroup = useMemo(() => {
    ...
    return Object.entries(grouped)
      .map(([group, rows]) => ({ group, rows: [...rows].sort((left, right) => (left?.rank || 0) - (right?.rank || 0)) }))
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {showStandingsTable ? (
              ...
              <View>
                ...
                {sortedStandings.length === 0 ? (
                  ...
                  <View key={`${row?.team?.id || row?.team?.name}-${row?.rank}`} style={styles.tableRow}>
                    ...
                    <Text style={[styles.rowCell, styles.colGoals]}>{row?.all?.goals?.for ?? '-'} / {row?.all?.goals?.against ?? '-'}</Text>
                    <Text style={[styles.rowCell, styles.colDiff]}>{typeof row?.goalsDiff === 'number' && row.goalsDiff > 0 ? `+${row.goalsDiff}` : (row?.goalsDiff ?? '-')}</Text>
                    <Text style={[styles.rowCell, styles.colPts, styles.pointsText]}>{row?.points ?? '-'}</Text>
                    <View style={[styles.colForm, styles.formCell]}>{renderForm(row?.form, styles, C)}</View>
                  </View>
                ))}
              </View>
            </ScrollView>
            ...
            {standingsByGroup.map((groupItem) => (
              <View key={groupItem.group} style={styles.groupBlock}>
                ...
                {groupItem.rows.map((row) => (
                  <View key={`${groupItem.group}-${row?.team?.id || row?.team?.name}-${row?.rank}`} style={styles.groupRow}>
                    <Text style={styles.groupRank}>{row?.rank ?? '-'}</Text>
                    <TeamLogo uri={row?.team?.logo} size={18} />
                    <Text style={styles.groupName} numberOfLines={1}>{row?.team?.name || '-'}</Text>
                    <Text style={styles.groupPoints}>{row?.points ?? '-'} pts</Text>
                  </View>
                ))}
              </View>
            ))}
          ...
          <View style={styles.card}>
            ...
            <Text style={styles.emptyText}>Aucun standings league disponible pour cette competition.</Text>
          </View>
        )}
      </ScrollView>
    </View>
...
const createStyles = (C) => StyleSheet.create({
  ...
  headerLogo: { backgroundColor: C.panel, borderWidth: 0 },
  ...
  loadingText: { marginTop: 10, color: C.muted, fontSize: 14, fontWeight: '700' },
  content: { padding: 16, paddingBottom: 28 },
  typeCard: { backgroundColor: C.panel, borderRadius: 18, borderWidth: 1, borderColor: C.border, padding: 14 },
  typeTitle: { color: C.muted, fontSize: 12, fontWeight: '800' },
  typeValue: { marginTop: 4, color: C.text, fontSize: 16, fontWeight: '900' },
  card: { marginTop: 12, backgroundColor: C.panel, borderRadius: 18, borderWidth: 1, borderColor: C.border, padding: 14 },
  sectionTitle: { color: C.text, fontSize: 14, fontWeight: '900', marginBottom: 10 },
  tableHeader: { flexDirection: 'row', paddingBottom: 8, borderBottomWidth: 1, borderBottomColor: C.border },
  tableRow: { flexDirection: 'row', paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: C.border, alignItems: 'center' },
  headCell: { color: C.muted, fontSize: 11, fontWeight: '900' },
  rowCell: { color: C.text, fontSize: 12, fontWeight: '700' },
  ...
  teamName: { color: C.text, fontSize: 12, fontWeight: '800', flex: 1 },
  pointsText: { fontWeight: '900', color: C.accent },
  formCell: { flexDirection: 'row' },
  formWrap: { flexDirection: 'row', alignItems: 'center', gap: 5 },
  formDot: { width: 9, height: 9, borderRadius: 999 },
  formEmpty: { color: C.muted, fontSize: 12, fontWeight: '700' },
  groupBlock: { marginTop: 8 },
  groupTitle: { color: C.accent, fontSize: 13, fontWeight: '900', marginBottom: 8 },
  groupRow: { flexDirection: 'row', alignItems: 'center', gap: 8, paddingVertical: 10, borderTopWidth: 1, borderTopColor: C.border },
  groupRank: { width: 24, color: C.muted, fontWeight: '900' },
  groupName: { flex: 1, color: C.text, fontWeight: '800' },
  groupPoints: { color: C.accent, fontWeight: '900' },
  emptyText: { color: C.muted, fontSize: 12, fontWeight: '700', paddingVertical: 4 },
});
